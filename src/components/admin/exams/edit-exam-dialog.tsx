...
import { CreateExamQuestionData, ExamWithDetails } from '@/types/exam'
...
export function EditExamDialog({ exam, open, onOpenChange }: EditExamDialogProps) {
  ...
  const updateQuestion = (questionIndex: number, updates: Partial<CreateExamQuestionData>) => {
    setQuestions(questions.map((q, index) => (index === questionIndex ? { ...q, ...updates } : q)))
  }
                    ...
                    <div className="space-y-2">
                      ...
                      {questions.length === 0 && (
                        <p className="text-muted-foreground text-sm">
                          No se han agregado preguntas aún
                        </p>
                      )}
                    </div>
                  </CardContent>
