...
// Jitsi library types - no TypeScript definitions available
...
export function JitsiProvider({ children }: { children: React.ReactNode }) {
        ...
        connection.addEventListener(
          ...
          () => {
            ...
            createLocalTracks().then((tracks) => {
              const conference = connection.initJitsiConference(conferenceRoomName, {
                p2p: { enabled: false },
              })
              conferenceRef.current = conference

              conference.on(
                JitsiMeetJSRef.current.events.conference.TRACK_ADDED,
                (track: JitsiTrack) => {
                  if (!track.isLocal()) {
              ...
              conference.on(
                JitsiMeetJSRef.current.events.conference.TRACK_REMOVED,
                (track: JitsiTrack) => {
                  if (!track.isLocal()) {
                    const id = track.getParticipantId()
              ...
              conference.on(
                JitsiMeetJSRef.current.events.conference.USER_JOINED,
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                (id: string, user: any) => {
  ...
  const toggleScreenShare = async () => {
    ...
    if (isScreenSharing) {
      // Stop sharing
      const desktopTrack = rawLocalTracks.find(
        (t) => t.getType() === 'video' && t.videoType === 'desktop'
      )
      if (desktopTrack) {
      ...
      try {
        ...
        if (desktopTrack) {
          desktopTrack.addEventListener(
            JitsiMeetJSRef.current.events.track.TRACK_MUTE_CHANGED,
            () => {
  ...
  // Command handling
  const sendCommand = (name: string, values: Record<string, unknown>) => {
  ...
  const addCommandListener = (command: string, handler: (values: Record<string, unknown>) => void) => {
    if (conferenceRef.current) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  const removeCommandListener = (command: string, handler: (values: Record<string, unknown>) => void) => {
    if (conferenceRef.current) {
