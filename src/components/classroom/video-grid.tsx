...
import { MicOff, VideoOff, MessageSquare } from 'lucide-react'
...
function VideoTile({ track, isTeacher }: { track: VideoTrack; isTeacher: boolean }) {
    ...
    React.useEffect(() => {
        if (track?.videoTrack && videoRef.current) {
            const jitsiTrack = track.videoTrack as any
            jitsiTrack.attach(videoRef.current)
            return () => {
                jitsiTrack.detach(videoRef.current)
            }
        }
    }, [track?.videoTrack])
...
export function VideoGrid({ localTrack, remoteTracks = [], isTeacher }: VideoGridProps) {
    ...
    const mockTeacher: VideoTrack = {
        ...
        isLocal: false,
        isMuted: false,
        isVideoMuted: true, // Mocking video off to show avatar
        isTeacher: true
    } as any
    ...
    const teacherTrack = isTeacher
        ...
        : (remoteTracks.find(t => (t as any).isTeacher) || safeTeacherMock)
